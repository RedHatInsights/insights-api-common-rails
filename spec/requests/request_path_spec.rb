RSpec.describe "ManageIQ::API::Common::ApplicationController Request path", :type => :request do
  ...
  it "invalid ID (only string characters)" do
    ...
    expect(response.parsed_body).to eq("errors" => [{"detail" => "ManageIQ::API::Common::ApplicationControllerMixins::RequestPath::RequestPathError: ID is invalid", "status" => 400}])
  end
