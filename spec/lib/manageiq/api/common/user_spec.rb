describe ManageIQ::API::Common::User do
  ...
  context "ManageIQ::API::Common::User" do
    it "raises an exception if Request.current is not set" do
      ManageIQ::API::Common::Request.current = nil
      expect { ManageIQ::API::Common::Request.current.user.username }.to raise_exception(NoMethodError)
    end
  end
...
describe ManageIQ::API::Common::User do
  ...
  around do |example|
    ManageIQ::API::Common::Request.with_request(request_bad) { example.call }
  end
