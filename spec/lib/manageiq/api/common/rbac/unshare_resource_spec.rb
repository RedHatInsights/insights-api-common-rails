describe ManageIQ::API::Common::RBAC::UnshareResource do
  ...
  shared_examples_for "#unshare" do
    it "remove resource definitions" do
      allow(ManageIQ::API::Common::RBAC::Service).to receive(:paginate).with(api_instance, :list_groups, {}).and_return(groups)
      allow(ManageIQ::API::Common::RBAC::Service).to receive(:paginate).with(api_instance, :list_roles, pagination_options).and_return([role1, role2])
      allow(ManageIQ::API::Common::RBAC::Service).to receive(:paginate).with(api_instance, :list_policies, {}).and_return(policies)
  ...
  context "invalid group uuid" do
    ...
    it "raises exception if group id is invalid" do
      allow(ManageIQ::API::Common::RBAC::Service).to receive(:paginate).with(api_instance, :list_groups, {}).and_return(groups)
      expect { subject.process }.to raise_exception(ManageIQ::API::Common::InvalidParameter)
    end
  end
